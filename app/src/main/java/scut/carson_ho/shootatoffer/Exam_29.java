...
public class Exam_29 {
    ...
    public static void printMatrixClockWisely(int[][] numbers) {
        ...
        // 矩阵的列数 = numbers.length、列号最大 = (numbers.length-1)/2
        // 矩阵的行数 = numbers[0].length、行号最大 = (numbers[0].length-1)/2
        while (numbers.length > (x * 2)   &&  numbers[0].length > (y * 2) ) {
    ...
    public static void printMatrixInCircle(int[][] numbers, int x, int y) {
        ...
        int rows = numbers.length; // 数组的行数 = 矩阵行数
        int cols = numbers[0].length; // 数组的列数 = 矩阵列数
